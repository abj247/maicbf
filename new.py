...
def main():
    ...
    for istep in range(config.EVALUATE_STEPS):
        ...
        def setup_mpc(model,s_np_i, s_np):
            ...
            @jit
            def combined_cbf_constraint_jax(h_values, kappa=10):
                # h_values should be a list or array of individual h_i(x)
                exp_sum = jnp.sum(jnp.exp(-kappa * jnp.array(h_values)))
                combined_h = -jnp.log(exp_sum)
                return combined_h
            ...
            kappa = 50
            h_combined = combined_cbf_constraint_jax(constraints_jax, kappa)
       ...
       # LQR 
        ...
        for t in range(scene.steps):
            ...
            for i in range(config.INNER_LOOPS_EVAL):
                ...
                s_traj.append(np.expand_dims(s_np[:, [0, 1, 2, 6, 7]], axis=0))
        #safety_info_baseline = np.zeros(args.num_agents, dtype=np.float32)
        # for t in range(scene.steps):
        #     # the goal states
        #     s_ref_np = np.concatenate(
        #         [scene.waypoints[t], np.zeros((args.num_agents, 5))], axis=1)
        ...
        #         individual_safety = safety_ratio == 1
        #         safety_baseline.append(individual_safety)
        #         safety_info_baseline = safety_info_baseline + individual_safety - 1
        #         safety_ratio_baseline = np.mean(individual_safety)
        #         safety_ratios_epoch_baseline.append(safety_ratio_baseline)
                #accuracy_lists.append(acc_list_np)
        ...
        # end_time = time.time()
            ...
            #     safety_ratio = 1 - np.mean(
            #         core.dangerous_mask_np(s_np, config.DIST_MIN_CHECK), axis=1)
            #     individual_safety_baseline = safety_ratio == 1
            #     safety_baseline.append(individual_safety_baseline)
            #     safety_info_baseline = safety_info_baseline + individual_safety_baseline - 1
            #     safety_ratio_baseline= np.mean(individual_safety_baseline)
            #     safety_ratios_epoch_baseline.append(safety_ratio_baseline)
            #     s_traj.append(np.expand_dims(s_np[:, [0, 1, 2, 6, 7]], axis=0))
            #     #accuracy_lists.append(acc_list_np)
            ...
            #     deadlock_ratio_baseline = deadlock_info_baseline / args.num_agents  # Adjust calculation for percentage of deadlock
            #     #print("deadlock_ratio", deadlock_ratio)
              
            #     deadlock_ratios_epoch_baseline.append(deadlock_ratio_baseline)  # Ensure values are <= 1
            #     #deadlock_rate_baseline = np.sum(deadlock_ratios_epoch_baseline)
            #     #leng_baseline   = len(deadlock_ratios_epoch_baseline)
            #     #print("length", leng)
            #     #print("deadlock_rate", deadlock_rate)
            #     #print("deadlock_ratios_epoch", len(deadlock_ratios_epoch))
            ...
            #     # collisions = core.dangerous_mask_np(s_np, config.DIST_MIN_CHECK)
            #     # collision_tracking[current_step_index] = np.any(collisions, axis=1).astype(int)
            #     #deadlock_tracking[current_step_index] = deadlock_mask.astype(int)
