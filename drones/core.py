...
def loss_barrier_ic(u, u_max = 0.8, indices=None, eps=[5e-2, 1e-3]):
    ...
    Args:
        ...
        eps (2, ): The margin factors.
    Returns:
        loss_dang_ic (float): The barrier loss for dangerous states with input constraints.
        loss_safe_ic (float): The barrier loss for safe sates with input constraints.
    ...
    loss_dang_ic = tf.reduce_sum(
        tf.math.maximum(h_u + eps[0], 0)) / (1e-5 + num)
    loss_safe_ic = tf.reduce_sum(
        tf.math.maximum(-h_u + eps[1], 0)) / (1e-5 + num)

    acc_dang_ic = tf.reduce_sum(tf.cast(
        tf.less_equal(h_u, 0), tf.float32)) / (1e-5 + num)
    acc_safe_ic = tf.reduce_sum(tf.cast(
        tf.greater_equal(h_u, 0), tf.float32)) / (1e-5 + num)

    acc_dang_ic = tf.cond(
        tf.greater(num, 0), lambda: acc_dang_ic, lambda: -tf.constant(1.0))
    acc_safe_ic = tf.cond(
        tf.greater(num, 0), lambda: acc_safe_ic, lambda: -tf.constant(1.0))

    return loss_dang_ic, loss_safe_ic, acc_dang_ic, acc_safe_ic
